...
def generate_paper_links(base_url, start_year, end_year, paper_code):
    sessions = {'Oct-Nov': 'w', 'March': 'm', 'May-June': 's', 'Feb-March': 'm'}
    links = []
    for paper in range(1,6):
        for year in range(start_year, end_year + 1):
            for session_name, session_code in sessions.items():
                # Add `_42` explicitly to target the correct file
                file_name = f"0625_{session_code}{str(year)[-2:]}_{paper_code}_4{paper}.pdf"
    ...
    for year, session_name, link in links:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                pdf_name = f"{session_name.replace('-', ' ')}{code}  .pdf"
...
def main():
    ...
    if choice == "1":
        ...
        links = generate_paper_links(base_url, start_year, end_year, paper_code="qp")
